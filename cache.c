...
char Read_Data_From_Cache(unsigned address){
    ...
    int set_num = ((address >> 5) & 0x7);
    int line_num = address & 0x1F;
    int tag = ((address >> 5) & 0xFFFFFF);
    int valid = 0; 
    int i = 0;

    while(i<1){
        if(cache.set[set_num].line[i].valid == 1 && cache.set[set_num].line[i].tag == tag){
            valid = 1;
            hit_count++;
            return cache.set[set_num].line[i].data[line_num];
        }
        i++;
    }
    ...
    if(valid == 0){
        ...
        cache.set[set_num].line[0].valid = 1;
        cache.set[set_num].line[0].tag = tag;
        for(int i = 0; i < 32; i++){
            cache.set[set_num].line[0].data[i] = system_bus[i];
        }
        return cache.set[set_num].line[0].data[line_num];
    }
